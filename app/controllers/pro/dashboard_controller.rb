...
class Pro::DashboardController < Pro::ProController
  ...
  def index
    ...
    @admins   = Admin  .where{created_at > Date.today - 2.month - 15.days}.count(:order => "DATE(created_at) ASC", :group => ["DATE(created_at)"])
    @comments = Comment.where{created_at > Date.today - 2.month - 15.days}.count(:order => "DATE(created_at) ASC", :group => ["DATE(created_at)"])

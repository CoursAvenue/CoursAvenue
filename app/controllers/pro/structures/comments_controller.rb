...
class Pro::Structures::CommentsController < Pro::ProController
  ...
  def highlight
    ...
    redirect_to pro_structure_comments_path(@structure),
      notice: "Le titre de l'avis a bien été utilisé en accroche"
  end
  ...
  def ask_for_deletion
    ...
    redirect_to pro_structure_comments_path(@structure),
      notice: "L'avis est en attente de suppression"
  end
    ...
    respond_to do |format|
      if @comment.destroy
        format.html { redirect_to pro_structure_comments_path(@structure),
                        notice: 'Votre avis a bien été supprimé' }
      else
        format.html { redirect_to (request.referrer || root_path),
                        alert: 'Vous ne pouvez pas supprimer ce avis' }
      end
    end
  end

# encoding: utf-8
class ::Pro::Structures::InvitedUsersController < Pro::ProController
  before_action :authenticate_pro_admin!
  load_and_authorize_resource :structure, find_by: :slug

  def index
    @structure        = Structure.friendly.find params[:structure_id]
    @invited_teachers = @structure.invited_users
  end
  ...
  def bulk_create
    ...
    respond_to do |format|
      format.html { redirect_to new_pro_structure_invited_user_path(@structure), notice: (params[:emails].present? ? 'Les autres professeurs ont bien été notifiés.': nil)}
    end
  end
end

...
class Structures::CommentsController < ApplicationController
  ...
  def index
    ...
    if params[:certified] == "true"
      @comments = @structure.comments.includes(:reply, :commentable, :user).accepted.certified.page(params[:page] || 1).per(5)
    elsif params[:certified] == "false"
      @comments = @structure.comments.includes(:reply, :commentable, :user).accepted.not_certified.page(params[:page] || 1).per(5)
    else
      @comments = @structure.comments.includes(:reply, :commentable, :user).accepted.page(params[:page] || 1).per(5)
    end
  ...
  def create_user(comment_params)
    ...
    elsif @comment.structure.present? and @user.subjects?

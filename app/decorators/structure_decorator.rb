class StructureDecorator < Draper::Decorator
  ...
  def places_popover()
    ...
    object.places(include: :city).each do |place|
      next if place.nil?
  ...
  def promotion_popover
    courses = object.courses(include: :prices).regulars.select{ |course| course.has_promotion? }
    output  = ''
    ...
    trainings = object.courses(include: :prices).trainings.select{ |course| course.has_promotion? }

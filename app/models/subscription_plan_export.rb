    ...
    workbook.add_worksheet(name: 'Suivi Premium') do |sheet|
      ...
      subscriptions.each do |subscription|
        ...
        sheet.add_row [
          ...
          client.impression_count(subscription.structure.id, since_date),
          client.view_count(subscription.structure.id, since_date),
          actions,
          conversations,
          client.telephone_count(subscription.structure.id, since_date),
          client.website_count(subscription.structure.id, since_date),
          Analytic::Client.score(actions, conversations),
          ...
          client.impression_count(subscription.structure.id),
          client.view_count(subscription.structure.id),
          client.action_count(subscription.structure.id),
          subscription.structure.main_contact.mailbox.conversations.where(mailboxer_label_id: Mailboxer::Label::INFORMATION.id).count,
          client.telephone_count(subscription.structure.id),
          client.website_count(subscription.structure.id)
        ]
      end
    end

class User < ActiveRecord::Base
  ...
  def self.from_omniauth(auth)
    ...
    u1 = where((User.arel_table[:provider].eq(auth.provider).and(User.arel_table[:uid].eq(auth.uid))).or(User.arel_table[:email].eq(auth.info.email))).first
    u2 = where((User.arel_table[:provider].eq(auth.provider).and(User.arel_table[:uid].eq(auth.uid)))).first
    Bugsnag.notify(RuntimeError.new("Facebook login -- User"), { auth: { u1: u1, u2: u2, 'omniauth.auth' => auth, provider: auth.provider, provider2: auth[:provider], provider3: auth['provider'] }} )

...
class SplitTheatreAndDance < ActiveRecord::Migration
  def up
    ...
    theatre = Subject.friendly.find 'theatre-danse'
    ...
    children_dance.each do |children_subject|
      child = Subject.friendly.find children_subject
      child.parent = dance
      child.save
    end
    ...
    children_theatre.each do |child_name|
      child = Subject.friendly.find child_name
      child.parent = theatre
      child.save
      child.children.create name: child.name
    end
    Subject.friendly.find('theatre').delete
